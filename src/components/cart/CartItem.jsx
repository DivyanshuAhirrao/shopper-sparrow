...
const CartItem = ({ props }) => {
  ...
  return (
    <section className="h-[180px] border-2 border-gray-200 w-[100%] flex">
      ...
      <div className="h-[100%] w-[78%] pl-4 py-1 flex flex-col justify-evenly">
        ...
        <p className="text-[12px]  text-gray-400">
          ...
          <span className="font-bold text-gray-600 text-[14px]">
           {((props.price - (props.price*(Math.round(props.discountPercentage) / 100)))*80).toFixed(2)}
          </span>{" "}
          <strike>{(props.price)*80}</strike>{" "}
          <span className="text-red-500 font-bold">{Math.round(props.discountPercentage)} %OFF</span>
        ...
        <article className="flex gap-12">
          ...
          <aside className="flex gap-2">
            ...
            <button onClick={decrement} className="bg-blue-200 p-1 px-2 rounded-md font-bold text-gray-600 text-[13.5px]">
              -
            </button>
            <p>{state}</p>
            <button onClick={increment} className="bg-blue-200 p-1 px-2 rounded-md font-bold text-gray-600 text-[13.5px]">
              +
            </button>
          </aside>
        </article>
      ...
      <IoMdClose className="pr-1 pt-1 text-[22px] hover:text-[25px] cursor-pointer" onClick={()=>removeProduct(id)}/>
    </section>
  );
};
